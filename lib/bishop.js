// create four arrays of the next diagonal square out to the edge
// (making sure to not go over 7 or less than 0 on the col or row)
...
export const bishopValidMoves = (currentBoard, currentPosition) => {
    ...
    const startingPostion = getPosition(currentBoard, row, col)
    const playerColor = getPosition(currentBoard, row, col).piece.color;
    const squareColor = startingPostion.color
