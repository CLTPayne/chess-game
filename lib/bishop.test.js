...
describe("#bishopValidMoves", () => {
	...
	it("only moves to squares the same colour when the starting square is white", () => {
		const firstMoveBoard = movePiece(currentBoard, [6, 4], [2, 4])
		const secondMoveBoard = movePiece(firstMoveBoard, [6, 6], [2, 6])
		const thirdMoveBoard = movePiece(secondMoveBoard, [7, 5], [3, 1])
		...
		const validMoveSquareColor = bishopValidMoves(thirdMoveBoard, currentPosition).filter(move => {
	...
	it("only moves to squares the same colour when the starting square is black", () => {
		const firstMoveBoard = movePiece(currentBoard, [6, 1], [2, 1])
		const secondMoveBoard = movePiece(firstMoveBoard, [6, 3], [2, 3])
		const thirdMoveBoard = movePiece(secondMoveBoard, [7, 2], [5, 4])
	...
	it("does not return a position occupied by the a piece belonging to the same player as a valid move", () => {
		...
		const thirdMoveBoard = movePiece(secondMoveBoard, [7, 2], [5, 4])
		const currentPosition = { row: 5, col: 4 }
		const whitePawnInPath = { row: 2, col: 1, piece: { type: "pawn", color: "white" }, color: "black" }
		expect(bishopValidMoves(thirdMoveBoard, currentPosition)).not.toContain(whitePawnInPath)
	})
	...
	it("does not return a position that is blocked by an occupied square", () => {
		...
		const secondMoveBoard = movePiece(firstMoveBoard, [6, 3], [2, 3])
		const thirdMoveBoard = movePiece(secondMoveBoard, [7, 2], [5, 4])
		const fourthMoveBoard = movePiece(thirdMoveBoard, [1, 7], [4, 5])
		const currentPosition = { row: 5, col: 4 }
		const blockedSquare = { row: 3, col: 6, piece: null, color: "black" }
		expect(bishopValidMoves(fourthMoveBoard, currentPosition)).not.toContain(blockedSquare)
	})
})
