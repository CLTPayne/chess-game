...
export const validateMove = (board, coordinates) => {
  ...
  const piece = _getPosition(board, startPosition[0], startPosition[1]).piece
  if (!piece) {
...
// Next Steps:
// Create basic logic for valid move. (not full legal chess moves per pieces, just moving a piece (instead of empty square))

// Refactor error to a validate move function in the js module
