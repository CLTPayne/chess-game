...
export const generateChessGame = () => ({
	history: [],
	lastMove: {
		from: null,
		to: null
...
export const capturePiece = (board, row, col) => {
...
// Option 1 - need to pass in the game as an additional parameter
// Option 1 - could pass in the game in stead of the board, and extract the board internally?
export const movePiece = (board, startCoordinate, endCoordinate) => {
	...
	// Option 1 - Grab the end position and store in the captured pieces object of the game
	endPosition.piece = startPosition.piece;
	endPosition.piece.hasMoved = true;
	// Option 1 - Grab the start position and store in the game.lastMove.from
	// Option 1 - Grab the end position and store in the game.lastMove.to
	startPosition.piece = null;
	return {
		newBoard,
		movedFrom: startPosition,
		movedTo: endPosition
	};
	// Option 2 - keep inputs the same and output an object that can destruct
	// the new board from as well as the start and end position and any pieces
	// taken to add to the
	// game object where currently just push board into history
};
