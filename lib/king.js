import { getPosition, getOppositionPositions, moves, cloneBoard } from "./chessboard.js";
...
export const kingValidMoves = (currentBoard, currentPosition) => {
    ...
    const opponentValidMoves = getOppositionPositions(currentBoard, playerColor)
        .map(element => element.piece.type !== 'king' && moves[element.piece.type] && moves[element.piece.type](currentBoard, element))
    ...
    for (const element of validMoves) {
        ...
        if (isValidMoveOccupiedSquare(element, validMoves)) {
            ...
            const validMovePosition = getPosition(newBoard, element.row, element.col);

            // delete the piece of the position as if taken by the king
            validMovePosition.piece = null;

            const opponentValidMovesIfPieceTaken = getOppositionPositions(newBoard, playerColor)
                .map(element => element.piece.type !== 'king' && moves[element.piece.type] && moves[element.piece.type](newBoard, element))
                .filter(element => element)
                .reduce((acc, value) => acc.concat(value), [])

            if (opponentValidMovesIfPieceTaken.length > opponentValidMoves.length) {
                movesInCheck.push(element)
            }
        }
    }
