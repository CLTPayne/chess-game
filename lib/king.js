...
export const kingValidMoves = (currentBoard, currentPosition) => {
    ...
    console.log({ opponentValidMoves })

    return validMoves;

    // loop over validMoves and filter out any move that is in check
    // ie compare the square with a separate array of squares that is created by calling valid moves on ALL opponent pieces. 
}