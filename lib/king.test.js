...
describe("#kingValidMoves", () => {
	...
	it("does not return a position that would be in check if the king moved and took a piece", () => {
		...
		const correctValidMoves = [
			{ col: 0, color: "black", piece: null, row: 3 },
			{ col: 1, color: "white", piece: null, row: 3 },
			{ col: 2, color: "black", piece: null, row: 5 },
			{ col: 0, color: "black", piece: null, row: 5 },
			{ col: 0, color: "white", piece: null, row: 4 }
		];
		expect(kingValidMoves(forthMoveBoard, currentPosition)).toStrictEqual(
			correctValidMoves
		);
	...
	it.only("does not return a position that would be in check from a pawn move, just a pawn capture", () => {
		...
		const correctValidMoves = [
			{ col: 5, color: "black", piece: null, row: 4 },
			{ col: 6, color: "black", piece: null, row: 5 },
			{ col: 4, color: "white", piece: null, row: 6 },
			{ col: 4, color: "black", piece: null, row: 5 }
		];
		expect(kingValidMoves(forthMoveBoard, currentPosition)).toStrictEqual(
			correctValidMoves
