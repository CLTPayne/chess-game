...
describe("#kingValidMoves", () => {
	...
	it("identifies a valid castling move for black king", () => {
		const currentBoard = []; // make this a board with lots of pieces gone? could be a fixture
		const currentPosition = { row: 0, col: 4 };
		const correctValidMoves = []; // array including the castling move
		expect(kingValidMoves(forthMoveBoard, currentBoard)).toStrictEqual(
			correctValidMoves
		);
	});
});
