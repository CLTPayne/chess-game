...
describe('#knightValidMoves', () => {
    ...
    it("does not return a position that is blocked by another piece of the same color", () => {
        const firstMoveBoard = movePiece(currentBoard, [7, 6], [5, 5]);
        const secondMoveBoard = movePiece(firstMoveBoard, [6, 6], [3, 4]);
        const thirdMoveBoard = movePiece(secondMoveBoard, [1, 7], [3, 6]);
        ...
        expect(knightValidMoves(thirdMoveBoard, currentPosition)).toStrictEqual(correctValidMoves);
    })
})