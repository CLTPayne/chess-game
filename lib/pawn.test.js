...
describe("#pawnValidMoves", () => {
	...
	it("does not identify the same player color pieces as a capture", () => {
		const firstMoveBoard = movePiece(currentBoard, [6, 5], [4, 5]);
		const secondMoveBoard = movePiece(firstMoveBoard, [6, 6], [4, 6]);
		const thirdMoveBoard = movePiece(secondMoveBoard, [7, 6], [3, 4]);
