...
describe("#rookValidMoves", () => {
	...
	it("identifies horizontal & vertical moves for a rook", () => {
		...
		expect(result).toStrictEqual(correctValidMoves);
	...
	it("does not return a position occupied by the a piece belonging to the same player as a valid move", () => {
		...
		const blackPawnInPlay = {
			row: 1,
			col: 4,
			piece: { type: "pawn", color: "black", hasMoved: true },
			color: "black"
		};
		expect(rookValidMoves(fourthMoveBoard, currentPosition)).toStrictEqual(
			correctValidMoves
		);
		expect(rookValidMoves(fourthMoveBoard, currentPosition)).not.toContain(
			blackPawnInPlay
		);
	});
