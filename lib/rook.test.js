...
describe('#rookValidMoves', () => {
    ...
    it('does not return a position occupied by the a piece belonging to the same player as a valid move', () => {
        ...
        const secontMoveBoard = movePiece(firstMoveBoard, [0, 0], [2, 4])
        const thirdMoveBoard = movePiece(secontMoveBoard, [1, 4], [5, 6])
        ...
        const blackPawnInPlay = { row: 1, col: 4, piece: { type: "pawn", color: "black" }, color: 'black' }
        expect(rookValidMoves(forthMoveBoard, currentPosition)).toStrictEqual(correctValidMoves)
        expect(rookValidMoves(forthMoveBoard, currentPosition)).not.toContain(blackPawnInPlay)
    })
